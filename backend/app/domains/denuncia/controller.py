...
from app.domains.denuncia.use_cases.get_denuncia_by_id import GetDenunciaByIdUseCase
from app.domains.denuncia.use_cases.get_denuncias_by_area import (
    GetDenunciasByAreaUseCase,
)
from app.domains.denuncia.use_cases.get_metrics_by_department import (
    GetMetricsByDepartmentUseCase,
)
...
class DenunciaController:
    def __init__(self, denuncia_repository, denunciante_repository=None):
        ...
        self.get_metrics_by_department_use_case = GetMetricsByDepartmentUseCase(
            denuncia_repository
        )
        self.get_most_recent_by_city_use_case = GetMostRecentByCityUseCase(
            denuncia_repository
        )
    ...
    def create_denuncia(
        ...
        return self.create_denuncia_use_case.execute(
            userid, category, content, totem, coordenadas, image
        )

    def get_denuncias_by_area(
        self, latitude: float, longitude: float, raio: float, categoria: str = None
    ):
        return self.get_denuncias_by_area_use_case.execute(
    ...
    def get_denuncia_by_id(self, usuario: str, data: str, coordenadas: str):
        denuncia = self.get_denuncia_by_id_use_case.execute(usuario, data, coordenadas)
        if not denuncia:
